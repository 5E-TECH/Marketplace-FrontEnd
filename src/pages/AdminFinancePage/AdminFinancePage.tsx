...
export default function AdminFinancePage() {
  ...
  return <main><PageHeader title={t('admin.finance.title')} description={t('admin.finance.description')} /><Tabs activeKey={tab} onChange={value => setTab(value as FinanceTab)} items={[{ key: 'payouts', label: t('admin.finance.payouts') }, { key: 'reports', label: t('admin.finance.report') }, { key: 'reconciliation', label: t('admin.finance.reconciliation') }]} />
    <FilterPanel className={styles.toolbar} aria-label={t('admin.common.filters')}><Input value={shopId} inputMode="numeric" allowClear placeholder={t('admin.finance.shopId')} aria-label={t('admin.finance.shopId')} onChange={event => { setShopId(event.target.value.replace(/\D/g, '')); setPage(1); }} />{tab === 'payouts' ? <Select<StatusFilter> value={status} aria-label={t('admin.finance.payoutStatus')} options={[{ value: 'ALL', label: t('admin.finance.allStatuses') }, ...payoutStatuses.map(value => ({ value, label: value }))]} onChange={value => { setStatus(value); setPage(1); }} /> : <><Input type="date" value={dateFrom} max={dateTo || undefined} aria-label={t('admin.finance.dateFrom')} onChange={event => setDateFrom(event.target.value)} /><Input type="date" value={dateTo} min={dateFrom || undefined} aria-label={t('admin.finance.dateTo')} onChange={event => setDateTo(event.target.value)} /></>}<Button icon={<RotateCcw size={16} />} disabled={!shopId && status === 'ALL' && !dateFrom && !dateTo} onClick={() => { setShopId(''); setStatus('ALL'); setDateFrom(''); setDateTo(''); setPage(1); }}>{t('adminOrders.clear')}</Button></FilterPanel>
    {tab === 'payouts' ? payouts.isPending ? <ContentState state="loading" /> : payouts.isError ? <ContentState state="error" description={getAuthErrorMessage(payouts.error)} onAction={() => void payouts.refetch()} /> : <div className={styles.table}><DataTable rowKey="id" columns={columns} dataSource={payouts.data.items} scroll={{ x: 820 }} emptyState={<EmptyState compact title={t('admin.finance.empty')} description={t('admin.finance.emptyDescription')} />} pagination={{ ...createTablePagination(20), current: page, total: payouts.data.total }} onChange={value => setPage(value.current ?? 1)} /></div> : activeReport.isPending ? <ContentState state="loading" /> : activeReport.isError ? <ContentState state="error" description={getAuthErrorMessage(activeReport.error)} onAction={() => void activeReport.refetch()} /> : <ReportView value={activeReport.data} />}
  </main>;
}
