...
import { App, Button, Form, Tag, Typography } from 'antd';
...
export default function UsersPage() {
  ...
  const columns: ColumnsType<ManagedUser> = [
    ...
    { title: t('users.actions'), width: 104, align: 'center', render: (_, user) => <span className={styles.rowActions}><Button type="text" icon={<Pencil size={16}/>} aria-label={t('users.edit')} onClick={() => setEditing(user)} /><Button type="text" danger icon={<Trash2 size={16}/>} aria-label={t('users.delete')} onClick={() => setDeleting(user)} /></span> },
  ];
