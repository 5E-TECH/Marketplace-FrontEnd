import { Alert, App, Button, DatePicker, Form, InputNumber, Switch, Tag, Upload } from 'antd';
import dayjs, { type Dayjs } from 'dayjs';
import { CalendarClock, ChevronDown, ChevronUp, GalleryHorizontalEnd, ImageUp, Link2, Pencil, Plus, Store, Trash2 } from 'lucide-react';
...
import { MAX_BANNERS, shopBannerLink, shopSlugFromBannerLink, type Banner, type BannerPayload } from '../../features/banners/model/bannerTypes';
import { AdminShopSelect, type ShopReference } from '../../features/adminShops/ui/AdminShopSelect';
...
interface BannerValues { shop: ShopReference | null; imageUrl: string; sortOrder: number; isActive: boolean; startsAt?: Dayjs | null; endsAt?: Dayjs | null }
...
/**
 * Banner do'konga bog'lanadi: sarlavha — do'kon nomi (backend majburiy
 * so'raydi, storefront rasm ustida ko'rsatadi), havola — do'kon sahifasi.
 */
function payload(values: Partial<BannerValues>): BannerPayload {
  return {
    title: values.shop?.name.trim() ?? '',
    imageUrl: values.imageUrl?.trim() ?? '',
    linkUrl: values.shop ? shopBannerLink(values.shop.slug) : null,
...
/** Tahrirda: `/dokon/:slug` havolali banner o'sha do'kon tanlangan holda ochiladi. */
function shopFromBanner(banner: Banner): ShopReference | null {
  const slug = shopSlugFromBannerLink(banner.linkUrl);
  return slug ? { slug, name: banner.title } : null;
}
...
export default function AdminBannersPage() {
  ...
  const title = Form.useWatch('shop', form)?.name;
  ...
  const openForm = (banner: Banner | null) => {
    ...
    setInitialValues(banner
      ? { shop: shopFromBanner(banner), imageUrl: banner.imageUrl, sortOrder: banner.sortOrder, isActive: banner.isActive, startsAt: banner.startsAt ? dayjs(banner.startsAt) : null, endsAt: banner.endsAt ? dayjs(banner.endsAt) : null }
      : { shop: null, imageUrl: '', sortOrder: nextOrder, isActive: true, startsAt: null, endsAt: null });
    setEditing(banner);
  };
  ...
  return <main className={styles.page}>
    ...
    {query.isPending ? <ContentState state="loading" /> : query.isError ? <ContentState state="error" title={t('admin.banners.loadError')} description={getApiErrorMessage(query.error)} onAction={() => void query.refetch()} /> : (
      <TablePanel title={t('admin.banners.title')} caption={t('pagination.total', { total: banners.length })}>
        {banners.length ? <ol className={styles.rows} aria-busy={query.isFetching || reorder.isPending}>
          {banners.map((banner, index) => {
            ...
            return <li className={styles.row} key={banner.id} data-banner-id={banner.id}>
              ...
              <div className={styles.copy}>
                ...
                {banner.linkUrl ? <span className={styles.link}>
                  {shopSlugFromBannerLink(banner.linkUrl) ? <Store size={13} aria-hidden /> : <Link2 size={13} aria-hidden />}
                  <span>{banner.linkUrl}</span>
                </span> : null}
                <span className={styles.meta}>
                  {statusTag(banner)}
                  {period ? <span className={styles.period}><CalendarClock size={13} aria-hidden />{period}</span> : null}
                </span>
              </div>
              <div className={styles.actions}>
                <IconActionButton icon={<ChevronUp size={17} />} label={t('admin.banners.moveUp', { name: banner.title })} disabled={index === 0 || reorder.isPending} onClick={() => move(index, -1)} />
                <IconActionButton icon={<ChevronDown size={17} />} label={t('admin.banners.moveDown', { name: banner.title })} disabled={index === banners.length - 1 || reorder.isPending} onClick={() => move(index, 1)} />
                <span className={styles.divider} aria-hidden />
                <IconActionButton icon={<Pencil size={16} />} label={t('admin.banners.editAria', { name: banner.title })} onClick={() => openForm(banner)} />
                <IconActionButton icon={<Trash2 size={16} />} danger label={t('admin.banners.deleteAria', { name: banner.title })} onClick={() => { setDeleteError(''); setDeleting(banner); }} />
              </div>
            </li>;
          })}
        </ol> : <EmptyState compact title={t('admin.banners.empty')} description={t('admin.banners.emptyDescription')} />}
      </TablePanel>
    )}
    <FormModal<BannerValues> title={t(editing ? 'admin.banners.edit' : 'admin.banners.new')} open={editing !== undefined} form={form} initialValues={initialValues} loading={busy} onCancel={() => { if (!upload.isPending) setEditing(undefined); }} onSubmit={values => {
      ...
      <Form.Item name="shop" label={t('admin.banners.shop')} extra={t('admin.banners.shopHint')} rules={[{ required: true, message: t('admin.banners.shopRequired') }]}>
        <AdminShopSelect placeholder={t('admin.banners.shopPlaceholder')} disabled={saving} />
      </Form.Item>
