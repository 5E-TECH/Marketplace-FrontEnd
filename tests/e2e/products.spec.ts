...
test('kategoriya ro‘yxatdan nomi bo‘yicha tanlanadi va POST /products ga ID sifatida ketadi', async ({ page }) => {
  ...
  // Ichma-ich kategoriya "— " prefiksi bilan chiqadi (createCategoryOptions
  // chuqurlikni shunday ko'rsatadi), shuning uchun nom aynan shu ko'rinishda.
  await page.getByLabel('Kategoriya').click();
  await page.getByRole('option', { name: '\u2014 Smartfonlar' }).click();
